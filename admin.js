...
function smRenderSettings() {
  ...
  smRenderBrandingPreview();
}

// ---------- Брендинг (логотип/баннер в Firebase Storage) ----------
function smRenderBrandingPreview() {
  const branding = smGetBranding();
  const logoPreview = document.getElementById('sm-brand-logo-preview');
  const bannerPreview = document.getElementById('sm-brand-banner-preview');
  if (logoPreview && branding.logoUrl) logoPreview.src = branding.logoUrl;
  if (bannerPreview && branding.bannerUrl) bannerPreview.src = branding.bannerUrl;
}

function smBindBrandingUpload(inputId, statusId, previewId, field) {
  ...
  input.addEventListener('change', function () {
    ...
    if (!file) return;
    status.textContent = 'Загрузка в Firebase Storage…';
    status.classList.remove('error');

    smUploadBrandingAsset(file, field === 'logoUrl' ? 'logo' : 'banner').then(function (url) {
      status.textContent = 'Загружено ✓ — применено на всех страницах';
      if (preview) preview.src = url;
      const branding = smGetBranding();
      branding[field] = url;
      branding.updatedAt = Date.now();
      smSetBranding(branding);
      if (typeof smApplyBranding === 'function') smApplyBranding();
    }).catch(function (err) {
...
document.addEventListener('DOMContentLoaded', function () {
  try {
    ...
    smRenderBrandingPreview();
    smBindBrandingUpload('f-brand-logo', 'sm-brand-logo-status', 'sm-brand-logo-preview', 'logoUrl');
    smBindBrandingUpload('f-brand-banner', 'sm-brand-banner-status', 'sm-brand-banner-preview', 'bannerUrl');
