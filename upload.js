...
/**
 * Загружает логотип или баннер сайта в Firebase Storage (по фиксированному
 * пути — новая загрузка перезаписывает предыдущий файл, старые версии не
 * копятся) и возвращает Promise<string> со ссылкой на файл. Ссылка потом
 * сохраняется в Firestore (meta/branding) через smSetBranding — так её
 * получают все страницы и все устройства, без повторной загрузки в админке.
 *
 * Если Firebase Storage недоступен (нет плана Blaze/нет интернета) —
 * откатывается на imgbb, как остальные картинки в проекте, чтобы функция
 * не переставала работать целиком.
 *
 * @param {File} file
 * @param {'logo'|'banner'} kind
 */
async function smUploadBrandingAsset(file, kind) {
  if (!file) throw new Error('Файл не выбран.');
  if (!file.type.startsWith('image/')) throw new Error('Можно загружать только изображения.');
  if (file.size > 8 * 1024 * 1024) throw new Error('Файл слишком большой (максимум 8 МБ).');
  ...
  if (typeof smStorage !== 'undefined' && smStorage && typeof smFirebaseReady !== 'undefined' && smFirebaseReady) {
    try {
      ...
      const ref = smStorage.ref().child(path);
      const snapshot = await ref.put(file);
      // Кэш-бастинг: добавляем метку времени к ссылке, чтобы браузер
      // подхватил новую картинку сразу, а не показывал старую из кэша.
      const url = await snapshot.ref.getDownloadURL();
      return url + (url.indexOf('?') === -1 ? '?' : '&') + 'v=' + Date.now();
    } catch (e) {
      console.warn('[upload] Firebase Storage недоступен для брендинга, используем imgbb:', e);
    }
  }

  return smUploadImage(file);
}
