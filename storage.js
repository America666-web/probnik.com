...
// ---------- Брендинг (логотип и баннер, загружаются один раз из админки
// в Firebase Storage, ссылка хранится в Firestore и разлетается на все
// устройства/страницы) ----------
function smGetBranding() {
  return JSON.parse(localStorage.getItem(SM_KEYS.branding) || '{}');
}
function smSetBranding(branding) {
  smSafeSet(SM_KEYS.branding, JSON.stringify(branding));
  smSyncSetDocSafe('meta/branding', branding);
}
